...
class QNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim):
        ...
        self.gru1 = nn.GRU(hidden_dim, hidden_dim, num_layers= 1, batch_first= True)
    ...
    def forward(self, state_action_packed, hidden):
        ...
        gru_branch_1 = F.relu(self.linear2(xu_p))
        gru_branch_1 = pack_padded_sequence(gru_branch_1, seq_lens, batch_first= True, enforce_sorted= False)
        gru_branch_1, hidden_out_1 = self.gru1(gru_branch_1, hidden)
        gru_branch_1, _ = pad_packed_sequence(gru_branch_1, batch_first= True)

        x1 = torch.cat([fc_branch_1, gru_branch_1], dim=-1)
        ...
        gru_branch_2 = F.relu(self.linear6(xu_p))
        gru_branch_2 = pack_padded_sequence(gru_branch_2, seq_lens, batch_first= True, enforce_sorted= False)
        gru_branch_2, hidden_out_2 = self.gru2(gru_branch_2, hidden)
        gru_branch_2, _ = pad_packed_sequence(gru_branch_2, batch_first= True)

        x2 = torch.cat([fc_branch_2, gru_branch_2], dim=-1)
...
class GaussianPolicyGRU(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, action_space=None):
        ...
        self.linear1 = nn.Linear(num_inputs, hidden_dim)
        self.gru = nn.GRU(num_inputs, hidden_dim, num_layers= 1, batch_first= True)
    ...
    def forward(self, state, h_prev, sampling):
        ...
        if sampling == True:
            ...
            gru_branch, h_current = self.gru(state, h_prev)
        else:
            state_pad, _ = pad_packed_sequence(state, batch_first= True)
            fc_branch = F.relu(self.linear1(state_pad))
            gru_branch, h_current = self.gru(state, h_prev)
            gru_branch, seq_lens = pad_packed_sequence(gru_branch, batch_first= True)

        x = torch.cat([fc_branch, gru_branch], dim=-1)
        x = F.relu(self.linear2(x))
        ...
        return mean, log_std, h_current

    def sample(self, state, h_prev, sampling):

        mean, log_std, h_current = self.forward(state, h_prev, sampling)
        ...
        return action, log_prob, mean, h_current
