...
from SAC.utils1 import soft_update, hard_update
from model_GRU import GaussianPolicy, GaussianPolicyGRU, QNetwork, DeterministicPolicy
from torch.nn.utils.rnn import pack_padded_sequence, pad_packed_sequence, pad_sequence


class SAC_GRU(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicyGRU(num_inputs, action_space.shape[0], args.hidden_size, action_space=None).to(self.device)
    ...
    def select_action(self, state, h_prev, evaluate=False):
        ...
        if evaluate is False:
            action, _, _, h_current = self.policy.sample(state, h_prev, sampling= True)
        else:
            _, _, action, h_current = self.policy.sample(state, h_prev, sampling= True)
    ...
    def update_parameters(self, policy_memory, policy_batch_size, updates):
        ...
        hidden_out = (hidden_out.to(self.device))
        ...
        with torch.no_grad():
            ...
            next_state_action_p, next_state_log_pi_p, _, _ = self.policy.sample(next_state_batch, h_prev=hidden_out, sampling= False)
        ...
        pi_action_bat_p, log_prob_bat_p, _, _, = self.policy.sample(state_batch, h_prev= hidden_in, sampling= False)
