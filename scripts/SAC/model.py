...
class GaussianPolicyLSTM(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, action_space=None):
        ...
        # self.speed_layer_dims = 6

        # self.linear_speed = nn.Linear(1, self.speed_layer_dims)

        self.linear1 = nn.Linear(num_inputs, hidden_dim)
        self.lstm = nn.RNN(hidden_dim, hidden_dim, batch_first=True)
        # self.linear2 = nn.Linear(hidden_dim, hidden_dim)

        # self.lstm = nn.RNN(num_inputs, hidden_dim, batch_first=True)
    ...
    def sample(self, state, h_prev, c_prev, sampling, len_seq=None):
        ...
        mean, log_std, h_current, c_current = self.forward(state, h_prev, c_prev, sampling, len_seq)
        ...
        return action, log_prob, mean, h_current, c_current, mask_seq
