...
import itertools
from files.model_utils import disable_control
import farms_pylog as pylog

import files.model_utils as model_utils
from scripts.Mouse_RL_Environment import Mouse_Env
from SAC.replay_memory import ReplayMemory, PolicyReplayMemory
import scipy.io
...
if __name__ == "__main__":
    ...
    for i_episode in itertools.count(1):
        ...
        for i in range(mouseEnv.timestep):
            ...
            kinematics_x.append(p.getLinkState(mouseEnv.model, 115)[0][0])
            kinematics_y.append(p.getLinkState(mouseEnv.model, 115)[0][1])
            kinematics_z.append(p.getLinkState(mouseEnv.model, 115)[0][2])

            #np.savetxt('kin_x.txt', kinematics_x)
            #np.savetxt('kin_y.txt', kinematics_y)
            #np.savetxt('kin_z.txt', kinematics_z)
